---
title: "16s amplicon cleanup in Cloquet DNA optimization trial"
output:
  html_notebook
---
...
```{r}
sapply(c("phyloseq", "ggplot2", "ggClusterNet", "dplyr", "microbiome", "ggpubr", "decontam", "rstatix"),
...
#subset_taxa(ps.16s, Family !="Mitochondria"|is.na(Family)) %>% #subset_taxa(., Order != "Chloroplast"|is.na(Order)) 
#subset_taxa(ps.16s,Family != "Mitochondria" &  Order != "Chloroplast" )

ps.bacteria  = subset_taxa(ps.bacteria, ! is.na(Kingdom)) # there was 9 asv NOT in Kingdom

saveRDS(ps.bacteria, "results/ps.bacteria.rds")
...
#ps.known = subset_taxa(ps.bacteria, !is.na(Phylum)) # 11749-132= 11617 was removed.
...
p <- ggplot(depth_track1, aes(x=nonbacterial_filtered, fill=Protocol))+
  geom_histogram()+
  scale_x_log10()+
...
p
ggsave("results/pictures/newfilter/Distribution_of_sample_sequencing_depth.pdf", p)

# Statistic of reads after removing non-bacterial reads
depth_track1 %>%
   # remove outliers samples in time 0 (pine log 06, pine log 07)
  #subset(! row.names(depth_track) %in% c("C0", "Q0", "C36","Q36", "C37", "Q37")) %>% 
  group_by(Tree_host, Protocol, Year_since_decay_start) %>%
  dplyr::summarise(mean(nonbacterial_filtered), sd(nonbacterial_filtered))

depth_track1 %>%
  # remove decayed samples in time 0 (pine log 06, pine log 07)
  #subset(! row.names(depth_track) %in% c("C36","Q36", "C37", "Q37")) %>% 
  filter(Sample_or_Control == "Sample") %>%
  mutate(status = ifelse(Year_since_decay_start %in% c(0), "non_decayed", "decayed")) %>%
  group_by(Tree_host, status) %>%
  dplyr::summarise(mean(nonbacterial_filtered), sd(nonbacterial_filtered))
...
#ps.stat <- subset_samples(ps.16s, ! sample_names(ps.16s) %in% c("C0", "Q0"))
...
taxa_groups<- dplyr::group_by(Taxonomies_order, Tree_host, Year_since_decay_start, Order)
taxa_sum <- dplyr::summarise(taxa_groups, mean(Abundance), sd(Abundance))
#taxa_sum[is.na(taxa_sum)] <- 0
head(taxa_sum)

colnames(taxa_sum) = c("Tree_host", "Year_since_decay_start", "ID","mean","sd")
taxa_sum <- arrange(taxa_sum,desc(mean))
taxa_sum$mean <- taxa_sum$mean *100
taxa_sum <- as.data.frame(taxa_sum)

write.csv(taxa_sum, "results/Order_stats.csv")

paste("The reads of ", taxa_sum[1,3], " is ",round(taxa_sum[1,4],2),"%",
      "(SD ",round(taxa_sum[1,5],3), ")", " for ", taxa_sum[1,1], " at year ",
       taxa_sum[1,2], sep = "")
paste("The reads of ", taxa_sum[2,3], " is ",round(taxa_sum[2,4],2),"%",
      "(SD ",round(taxa_sum[2,5],3), ")", " for ", taxa_sum[2,1], " at year ",
      taxa_sum[2,2], sep = "")

Order_stat1 <- Taxonomies_order %>%
  filter(Sample_or_Control == "Sample") %>%
  mutate(status = ifelse(Year_since_decay_start %in% c(0), "non_decayed", "decayed")) %>%
  ...
  as.data.frame()
write.csv(Order_stat1, "results/Order_stats_1.csv")
...
taxa_groups<- dplyr::group_by(Taxonomies_family, Tree_host, Year_since_decay_start, Family)
taxa_sum <- dplyr::summarise(taxa_groups, mean(Abundance), sd(Abundance))
#taxa_sum[is.na(taxa_sum)] <- 0
head(taxa_sum)
...
write.csv(taxa_sum, "results/Family_stats.csv")

paste("The reads of ", taxa_sum[3,3], " is ",round(taxa_sum[3,4],2),"%",
      "(SD ",round(taxa_sum[3,5],3), ")", " for ", taxa_sum[3,1], " at year ",
       taxa_sum[3,2], sep = "")
paste("The reads of ", taxa_sum[4,3], " is ",round(taxa_sum[4,4],2),"%",
      "(SD ",round(taxa_sum[4,5],3), ")", " for ", taxa_sum[4,1], " at year ",
      taxa_sum[4,2], sep = "")

Family_stat1 <- Taxonomies_family %>%
  filter(Sample_or_Control == "Sample") %>%
  mutate(status = ifelse(Year_since_decay_start %in% c(0), "non_decayed", "decayed")) %>%
  ...
  arrange(dplyr::desc(mean)) %>%
  as.data.frame()
write.csv(Family_stat1, "results/Family_stats_1.csv")

# How about we remove pine_06,pine_07 samples with blue stain fungi pre-colonization
# Statistical summary of ASV classified as choloroplast (Order = "Chloroplast" )
ps.stat <- subset_samples(ps.16s, ! sample_names(ps.16s) %in% c("C0", "Q0", "C36", "Q36",
                                                               "C37", "Q37"))
```
...
# 1.1 Sample Filtering--Remove samples with low- or poor-quality sequence data (samples with less than 100 bacterial reads).
...
# How many phylum would be present after filtering? #33
length(get_taxa_unique(ps2,taxonomic.rank="Phylum"))

# Create table, number of features for each phyla
table(tax_table(ps2)[,"Phylum"],exclude=NULL)

cat(ntaxa(ps.16s), "OTUs in original table.\n")
cat(ntaxa(ps.bacteria), "OTUs in taxnomic filtered table.\n")
...
c(sum(colSums(otu_table(ps.16s))), sum(colSums(otu_table(ps.bacteria))),
...
# Both Qiagen and CTAB group are nor normally distributed
...
res.fried <- depth_track3 %>% 
  #filter(Sample_or_Control == "Sample") %>%
  filter ( ! SampleID %in% c("Q04", "Q06", "Q27", "Q38", "Q39")) %>% ## Remove Q04, Q06, Q27, Q38, Q39 as the C04, C06, C27, C38, C39 are filterd out before
  friedman_test(quality_filtered ~ Protocol |Sample_name)
res.fried 

# Birch
depth_track3 %>% 
  filter(! SampleID %in% c("Q04", "Q06", "Q27", "Q38", "Q39")) %>% # Remove to fo the repeated measure as those samples are       # filterd out before
...
ggsave("results/pictures/newfilter/reads_vs_protocol.pdf", p)

# Remove the pair with one NA
depth_track3 %>%
    filter(Sample_or_Control == "Sample") %>% filter (!SampleID %in% c("Q27", "C27", "Q04", "C04", "Q06", "C06", "Q38", "C38", "Q39", "C39")) %>% 
    rstatix::t_test(quality_filtered ~ Protocol, p.adjust.method = "bonferroni", paired=T) %>%
    add_significance() 
```
...
depth_track3  %>%
  filter(Sample_or_Control == "Sample") %>%
  group_by(Tree_host, Year_since_decay_start) %>%
  rstatix::wilcox_effsize(quality_filtered ~ Protocol, paired = TRUE)
...
ggsave("results/pictures/newfilter/reads_vs_protocol_group_by.pdf", p)
...
### t or wilcoxon test to test the effect of DECAY STATUS on reading depth adter filtration
The goal is to assess the effect of decay status on the read depth.
```{r}
# Decay status effect
...
ggsave("results/pictures/newfilter/reads_vs_status.pdf", p, width = 3, height = 2.5)

# Summarise the sample counts with less than 1000 counts
sum(depth_track4[depth_track4$status == "non_decayed", ]$quality_filtered<1000, na.rm = T)
sum(depth_track4[depth_track4$status == "decayed", ]$quality_filtered<1000, na.rm = T)
...
p
ggsave("results/pictures/newfilter/Prevalence_10_versus_total_abundance.pdf", p)
...
p
ggsave("results/pictures/newfilter/track_for_all_count.pdf", p, width = 5, height = 2.5)
```
...
depth_track5 %>%
    # remove decayed samples in time 0 (pine log 06, pine log 07)
  #subset(! row.names(.) %in% c("C36","Q36", "C37", "Q37")) %>% 
  group_by(Tree_host, Year_since_decay_start) %>%
   dplyr::summarise(mean(nonbacterial_filtered/nonchim), sd(nonbacterial_filtered/nonchim))
...
each_count <- paste(repead,
                    "We obtained an average read count per sample of", 
                    round(mean(sample_sums(ps2)),0), "with a range bwtween", 
...
row.names(depth_tax) = depth_tax$Row.names
depth_tax$Row.names  = NULL
